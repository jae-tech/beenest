...
async function checkServers() {
  ...
  try {
    ...
    console.log(
      `✅ 프론트엔드 서버: ${frontendResponse.status} ${frontendResponse.statusText}`
    );
  } catch (error) {
...
async function checkAPIEndpoints() {
  ...
  const endpoints = [
    ...
    { method: "GET", path: "/suppliers", description: "거래처 목록" },
  ...
  for (const endpoint of endpoints) {
    try {
      ...
      if (response.status === 401) {
        console.log(
          `🔒 ${endpoint.method} ${endpoint.path}: 인증 필요 (예상됨)`
        );
      } else if (response.status === 200) {
        console.log(`✅ ${endpoint.method} ${endpoint.path}: 접근 가능`);
      } else {
        console.log(
          `⚠️  ${endpoint.method} ${endpoint.path}: ${response.status} ${response.statusText}`
        );
      }
    } catch (error) {
