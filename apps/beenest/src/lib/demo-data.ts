...
import type { Order, Product, Supplier } from "@/types/api";
...
export const addDemoOrder = (order: Partial<Order>) => {
  ...
  const newOrder = {
    ...
    orderNumber:
      order.orderNumber ||
      `ORD-${today}-${String(orders.length + 1).padStart(3, "0")}`,
    createdAt: new Date().toISOString(),
    updatedAt: new Date().toISOString(),
  };
...
export const addDemoTransaction = (transaction: any) => {
  ...
  const newTransaction = {
    ...
    transactionNumber:
      ...
      `TXN-${today}-${String(transactions.length + 1).padStart(3, "0")}`,
    transactionDate:
