...
export class SuppliersController {
  ...
  @ApiResponse({ status: 201, description: '거래처 생성 성공' })
  ...
  @ApiQuery({
    name: 'search',
    required: false,
    description: '검색어 (회사명, 코드, 담당자, 이메일)',
  })
  @ApiQuery({
    name: 'status',
    required: false,
    description: '상태 필터 (active, inactive, pending)',
  })
  ...
  @ApiResponse({ status: 200, description: '거래처 통계 조회 성공' })
  ...
  @ApiResponse({ status: 200, description: '거래처 상세 조회 성공' })
  @ApiResponse({ status: 404, description: '거래처를 찾을 수 없음' })
  ...
  @ApiResponse({
    status: 400,
    description: '연결된 상품이 있어 삭제할 수 없음',
  })
  ...
  @ApiOperation({ summary: '거래처에 상품 연결' })
